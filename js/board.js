...
function checkFilter(task, filter) {
  return (
    ...
    task["taskCategoryValue"].toLowerCase().includes(filter)
  );
}
...
function showMoveTo(event, index, category) {
  ...
  document.getElementById(`taskCard${index}`).innerHTML =
    renderMoveToInTaskCards(index);
  ...
  if (category == "toDo") {
    ...
    document
      .getElementById("moveAwaitFeedback")
